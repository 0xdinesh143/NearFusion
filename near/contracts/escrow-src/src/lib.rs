...
use near_sdk::{
    env, near_bindgen, AccountId, Promise, PromiseResult, NearToken,
...
impl EscrowSrc {
    ...
    /// Withdraw funds with secret (reveals the secret to enable cross-chain claim)
    /// This is called by the maker to claim NEAR tokens from EVM→NEAR swap
    pub fn withdraw(&mut self, secret: String) -> Promise {
        ...
        log!(
            "Escrow withdrawal by {} with secret: {}",
    ...
    /// This can be called by taker if maker doesn't withdraw in time
    ...
    pub fn rescue_funds(&mut self, recipient: AccountId) -> Promise {
        ...
        // Validate caller (should be taker for safety)
        assert_eq!(
            env::predecessor_account_id(),
            self.immutables.taker,
...
mod tests {
    ...
    fn test_successful_withdrawal() {
        ...
        let mut escrow = EscrowSrc::new(immutables);
        
        // Should be able to withdraw with correct secret
        assert!(CryptoUtils::verify_secret(secret, &escrow.immutables.hashlock));
        
        // Withdrawal should succeed (in real test would check Promise result)
        let _promise = escrow.withdraw(secret.to_string());
        
        assert!(matches!(escrow.state, EscrowState::Withdrawn));
        assert_eq!(escrow.revealed_secret, Some(secret.to_string()));
    }

    #[test]
    #[should_panic(expected = "Invalid secret")]
    fn test_withdrawal_with_wrong_secret() {
        ...
        escrow.withdraw("wrong_secret".to_string());
    }
} 